...
Thanks for helping make the library more practical, multilingual, and properly attributed. Contributions should add a real production workflow or improve an existing recipe in a testable way.

## Fastest way to contribute

The bestimage.ai team maintains this collection. Use the [Prompt Submission template](.github/ISSUE_TEMPLATE/prompt.yml) for one tested prompt. It asks for the information maintainers need to review the result without requiring you to edit repository files.
...
When the repository is published, submit tested examples through its Issues tab. For several prompts, a new language pack, documentation, or structural changes, open a pull request instead. Keep each submission reviewable and avoid mixing unrelated changes.
